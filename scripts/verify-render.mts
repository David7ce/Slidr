/**
 * Temporary verification script: renders all 9 slide types through the real
 * export pipeline (renderer -> wrapSlideHtml -> Puppeteer -> Sharp).
 */
...
async function main() {
  ...
  console.log("Themes loaded:", themes.map((t) => t.id).join(", "));

  const themeId = process.argv[2] || "swiss-grid";
  const theme = await getTheme(themeId);
  if (!theme) throw new Error(`Theme "${themeId}" not found`);

  console.log(`\nTheme: ${theme.name} (${theme.category})`);
  console.log("  palette:", JSON.stringify(theme.palette));
  console.log("  fonts:", JSON.stringify(theme.fonts));
  console.log("  rules:", theme.designRules.length);
  ...
  console.log(`\nOutput: ${OUT}`);
  // Release the headless browser so this process can exit.
  await closeBrowser();
}
